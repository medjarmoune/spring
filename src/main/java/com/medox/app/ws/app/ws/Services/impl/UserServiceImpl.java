...
public class UserServiceImpl implements UserService {
	...
	public UserDto createUser(UserDto user) {
		...
		if(checkUser != null) throw new RuntimeException("User Arleady exist!");

	
		user.getContact().setContactId(util.generateStringId(30));
		user.getContact().setUserDto(user);
		
		for (int i = 0; i < user.getAdress().size(); i++) {
			AdressDto adress = user.getAdress().get(i);
			adress.setUserDto(user);
			adress.setAdressId(util.generateStringId(30));
			user.getAdress().set(i, adress);
		}
	...
	public UserDto updateUser(String id, UserDto userDto) {
		...
		userEntity.setFirsName(userDto.getFirsName());
	...
	public List<UserDto> getUsers(int page, int limit, String search, int status) {
		...
		Page<UserEntity> userPage;//  = userRepository.findAllUserByCriteria(pageableRequest);
